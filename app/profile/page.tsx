...
export default function Profile() {
  ...
  function exportData() {
    const payload = { student: draft, report: repo.report(), chat: repo.chat() };
      ...
      <main className="mx-auto max-w-3xl p-4">
        <Card>
          ...
          <label className="my-3 block">
            ...
            <input
              className="mt-1 w-full rounded-xl border p-3"
              value={draft.academic.subjects.join(', ')}
              onChange={(event) =>
                setDraft({
                  ...draft,
                  academic: {
                    ...draft.academic,
                    subjects: event.target.value.split(',').map((item) => item.trim()),
                  },
                })
              }
            />
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <Button variant="ghost" onClick={clearChat}>Clear chat history</Button>
          </div>
          {status && (
            <p role="status" className="mt-3 text-sm font-semibold text-teal-700">
              {status}
            </p>
          )}
          <p className="mt-4 text-sm text-slate-600">
            Privacy notice: this MVP avoids unnecessary data collection and stores demo data locally in your browser.
          </p>
        </Card>
      </main>
    </Shell>
